...
def main():
    ...
    # 1. Equalized Odds
    eq_odds = calculate_equalized_odds(
        df["y_true"], df["y_pred"], df["gender"]
    )
    print("Equalized Odds (FPR/TPR by Group):")
    for group, metrics in eq_odds.items():
        print(
            f"  {group:>10}: FPR={metrics['FPR']:.3f}, TPR={metrics['TPR']:.3f}"
        )
    ...
    for group, rate in di_rates.items():
        if base_rate > 0:
            ...
            print(
                f"  {group:>10}: Rate={rate:.3f} | Impact Ratio (vs Male): {ratio:.3f}"
            )
