...
Usage:
    CLI mode  :  python -m ml.pipelines.inference.risk_assistant
    Gradio UI :  python -m ml.pipelines.inference.risk_assistant --ui
"""
...
def predict_risk(
    ...
    contribs = pd.Series(shap_vals, index=feat_names).sort_values(
...
def launch_ui():
    ...
    iface = gr.Interface(
        ...
        inputs=[
            gr.Slider(
                1, 13, step=1, value=7, label="Age Group (1=18-24 … 13=80+)"
            ),
            gr.Number(value=27.5, label="BMI"),
            gr.Radio(
                [("Yes", 1), ("No", 0)], value=0, label="High Blood Pressure"
            ),
            gr.Radio(
                [("Yes", 1), ("No", 0)],
                value=0,
                label="Smoker (100+ cigarettes ever)",
            ),
            gr.Radio(
                [("Yes", 1), ("No", 0)], value=0, label="High Cholesterol"
            ),
            gr.Radio(
                [("Active", 1), ("Inactive", 0)],
                value=1,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ui", action="store_true", help="Launch Gradio web UI"
    )
