...
def override_dependency():
    ...
    app.dependency_overrides[get_current_session_id] = (
        mock_get_current_session_id
    )
    yield
...
def test_get_sessions_empty(client: TestClient):
    response = client.get(
        "/api/v1/security/sessions", headers={"X-API-Key": "test-dev-api-key"}
    )
...
def test_get_security_events_empty(client: TestClient):
    response = client.get(
        "/api/v1/security/events", headers={"X-API-Key": "test-dev-api-key"}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 0
...
def test_get_devices_empty(client: TestClient):
    response = client.get(
        "/api/v1/security/devices", headers={"X-API-Key": "test-dev-api-key"}
    )
    assert response.status_code == 200
...
def test_delete_session_not_found(client: TestClient):
    response = client.delete(
        f"/api/v1/security/sessions/{uuid4()}",
        headers={"X-API-Key": "test-dev-api-key"},
    )
