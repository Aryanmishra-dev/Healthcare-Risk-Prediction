...
class TestModelRegistry:
    ...
    def test_registry_verify_script(self):
        ...
        # Check if we are using stub models (size < 1KB)
        model_path = os.path.join(ROOT, "ml", "models", "diabetes_xgboost.pkl")
        if os.path.exists(model_path) and os.path.getsize(model_path) < 1024:
            pytest.skip(
                "Using stub models; hashes will intentionally mismatch the registry"
            )

        result = subprocess.run(
            ["python", "-m", "ml.registry.model_registry", "verify"],
            cwd=ROOT,
            capture_output=True,
            text=True,
        )
...
class TestProductionReadiness:
    def test_readiness_endpoint(self, client):
        ...
        resp = client.get(
            "/api/v1/health/ready", headers={"X-API-Key": VALID_API_KEY}
        )
        assert resp.status_code == 200
    ...
    def test_nginx_dev_config_exists(self):
        ...
        assert os.path.isfile(
            nginx_conf
        ), "deployment/nginx/nginx.dev.conf not found"
    ...
    def test_docker_compose_no_exposed_backend_port(self):
        ...
        assert (
            '"8000:8000"' not in content
        ), "Port 8000 should not be published to host"
...
class TestABTesting:
    def test_register_experiment(self):
        from backend.app.services.ab_testing import ABRouter
    ...
    def test_summary(self):
        ...
        assert (
            summary["champion"]["count"] + summary["challenger"]["count"] == 10
        )
