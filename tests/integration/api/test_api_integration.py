...
class TestAPIAuthentication:
    ...
    def test_json_api_requires_auth(self, client):
        ...
        for endpoint in endpoints:
            ...
            payload = (
                DIABETES_PAYLOAD
                if "diabetes" in endpoint
                else (HEART_PAYLOAD if "heart" in endpoint else LUNG_PAYLOAD)
            )
    ...
    def test_json_api_rejects_invalid_key(self, client):
        ...
        response = client.post(
            "/api/v1/predict/diabetes", json=DIABETES_PAYLOAD, headers=headers
        )
        assert response.status_code == 401
        assert response.json()["detail"] == "Invalid or missing API Key"
    ...
    def test_json_api_accepts_valid_key(self, client):
        ...
        # Test Heart
        resp_heart = client.post(
            "/api/v1/predict/heart", json=HEART_PAYLOAD, headers=headers
        )
        assert resp_heart.status_code == 200
        assert "risk_percentage" in resp_heart.json()

        # Test Lung
        resp_lung = client.post(
            "/api/v1/predict/lung", json=LUNG_PAYLOAD, headers=headers
        )
...
class TestAuditLogging:
    ...
    def get_latest_log(self):
        ...
        if row:
            return {
                "disease_model": row.disease_model,
                "source": row.source,
