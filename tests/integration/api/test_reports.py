...
def test_get_reports_empty(client: TestClient):
    response = client.get("/api/v1/reports", headers={"X-API-Key": "test-dev-api-key"})
...
def test_get_report_not_found(client: TestClient):
    response = client.get(f"/api/v1/reports/{uuid4()}", headers={"X-API-Key": "test-dev-api-key"})
    assert response.status_code == 404

def test_delete_report_not_found(client: TestClient):
    response = client.delete(f"/api/v1/reports/{uuid4()}", headers={"X-API-Key": "test-dev-api-key"})
    assert response.status_code == 404
