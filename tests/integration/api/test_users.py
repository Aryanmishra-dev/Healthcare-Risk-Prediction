...
def test_get_dashboard(client: TestClient):
    response = client.get("/api/v1/users/dashboard", headers={"X-API-Key": "test-dev-api-key"})
    print(response.json())
...
def test_get_profile(client: TestClient):
    response = client.get("/api/v1/users/profile", headers={"X-API-Key": "test-dev-api-key"})
    assert response.status_code == 200
...
def test_get_settings(client: TestClient):
    response = client.get("/api/v1/users/settings", headers={"X-API-Key": "test-dev-api-key"})
    assert response.status_code == 200
...
def test_get_account(client: TestClient):
    response = client.get("/api/v1/users/account", headers={"X-API-Key": "test-dev-api-key"})
    assert response.status_code == 200
    assert "active_sessions_count" in response.json()

def test_get_statistics(client: TestClient):
    response = client.get("/api/v1/users/statistics", headers={"X-API-Key": "test-dev-api-key"})
    assert response.status_code == 200
