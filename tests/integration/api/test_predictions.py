...
def test_get_history_empty(client: TestClient):
    response = client.get("/api/v1/predictions/history", headers={"X-API-Key": "test-dev-api-key"})
...
def test_favorite_prediction_not_found(client: TestClient):
    response = client.post("/api/v1/predictions/999/favorite", headers={"X-API-Key": "test-dev-api-key"})
    assert response.status_code == 404

def test_delete_prediction_not_found(client: TestClient):
    response = client.delete("/api/v1/predictions/999", headers={"X-API-Key": "test-dev-api-key"})
    assert response.status_code == 404
