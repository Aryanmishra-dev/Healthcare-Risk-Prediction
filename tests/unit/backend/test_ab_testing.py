...
from backend.app.services.ab_testing import ABRouter, ab_router
...
def test_register_experiment(clean_router):
    clean_router.register(
        "test_model", dummy_champion, dummy_challenger, traffic_pct=20
    )
...
def test_register_experiment_invalid_pct(clean_router):
    with pytest.raises(
        ValueError, match="traffic_pct must be between 0 and 100"
    ):
        clean_router.register(
            "test_model", dummy_champion, dummy_challenger, traffic_pct=150
        )


def test_route_uses_deterministic_hashing(clean_router):
    clean_router.register(
        "test_model", dummy_champion, dummy_challenger, traffic_pct=50
    )
