...
class HealthPredictUser(HttpUser):
    ...
    @task(1)
    def v1_root(self):
        self.client.get("/api/v1/", headers=self.api_headers)
    ...
    def model_registry(self):
        self.client.get("/api/v1/models", headers=self.api_headers)

    # ── Diabetes predictions ──────────────────────────────────────────────
    @task(5)
    def predict_diabetes_json(self):
        self.client.post("/api/v1/predict/diabetes", headers=self.api_headers, json={
    ...
    def predict_heart_json(self):
        self.client.post("/api/v1/predict/heart", headers=self.api_headers, json={
            "age": random.randint(1, 13),
    ...
    def predict_lung_json(self):
        self.client.post("/api/v1/predict/lung", headers=self.api_headers, json={
