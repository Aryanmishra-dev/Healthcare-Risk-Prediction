...
| Path | Purpose |
| --- | --- |
| `backend/` | FastAPI app, API routes, schemas, services, database setup, and backend utilities. |
| `frontend/` | HTMX/Jinja templates, static assets, styles, and frontend scaffolding. |
| `ml/` | Training, evaluation, inference, feature store, model registry, and model artifacts. |
| `shared/` | Pure shared utilities, constants, and cross-runtime types. |
| `tests/` | Unit, integration, API, infrastructure, and load tests organized by scope. |
| `data/` | Local datasets restored through DVC or generated during development. |
| `config/` | Environment templates, central settings loader, and feature flags. |
| `deployment/` | Docker Compose, Kubernetes, Nginx, CI workflow templates, scripts, and infrastructure. |
