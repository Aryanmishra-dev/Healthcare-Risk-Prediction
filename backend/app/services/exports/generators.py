...
async def generate_user_data_json(
    ...
    export_data: Dict[str, Any] = {
        ...
        "reports": [],
        "notifications": [],
        "security": {"sessions": [], "login_history": [], "events": []},
    }
    ...
    pred_result = await db.execute(
        select(PredictionAuditLog).where(PredictionAuditLog.user_id == user_id)
    )
    ...
    rep_result = await db.execute(
        select(UserReport).where(
            UserReport.user_id == user_id, UserReport.deleted_at.is_(None)
        )
    )
