...
class ExportService:
    ...
    async def request_export(
        self, db: AsyncSession, user_id: uuid.UUID, export_format: str = "json"
    ) -> DataExport:
    ...
    async def process_export_task(
        ...
        try:
            ...
            export_record.expires_at = utc_now() + timedelta(
                days=7
            )  # Expires in 7 days
            await db.commit()
    ...
    async def get_exports(
        self, db: AsyncSession, user_id: uuid.UUID, params: ExportQueryParams
    ) -> PaginatedExportResponse:
        ...
        query = (
            query.order_by(desc(DataExport.created_at))
            .offset(offset)
            .limit(params.size)
        )
    ...
    async def get_export(
        self, db: AsyncSession, user_id: uuid.UUID, export_id: uuid.UUID
    ) -> DataExport:
        export_record = await db.get(DataExport, export_id)
        ...
        if (
            export_record.status == "completed"
            and export_record.expires_at
            and export_record.expires_at < utc_now()
        ):
