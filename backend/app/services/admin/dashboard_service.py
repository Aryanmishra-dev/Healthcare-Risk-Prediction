...
class AdminDashboardService:
    ...
    async def get_overview(db: AsyncSession) -> Dict[str, Any]:
        ...
        The @cached decorator uses the method name and kwargs (if any) as cache
        key. Since db is an object, it won't be included in the cache key
        directly. Wait, our cache decorator currently excludes complex objects,
        which is correct.
        """
    ...
    async def get_charts(db: AsyncSession) -> Dict[str, Any]:
        """Get charting data (predictions over time, disease
        distribution, etc.)."""
        predictions_trend = (
