...
class AuthorizationService:
    @staticmethod
    def can(user: User, permission: str, tenant_id: Optional[Any] = None) -> bool:
        """
        ...
        if tenant_id:
            ...
            membership = next((m for m in getattr(user, 'memberships', []) if str(m.tenant_id) == str(tenant_id)), None)
            if membership:
