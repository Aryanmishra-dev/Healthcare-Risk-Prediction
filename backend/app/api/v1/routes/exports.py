...
from backend.app.schemas.export import ExportRequest, ExportResponse, PaginatedExportResponse, ExportQueryParams
...
async def request_export(
    ...
    try:
        export_record = await export_service.request_export(db, current_user.id, payload.export_format)
        background_tasks.add_task(export_service.process_export_task, db, export_record.id)
...
async def download_export(
    ...
    if export_record.status != "completed":
        raise HTTPException(status_code=400, detail=f"Export is not ready for download. Status: {export_record.status}")
    ...
    content_type = "application/json" if export_record.export_format == "json" else "text/csv"
