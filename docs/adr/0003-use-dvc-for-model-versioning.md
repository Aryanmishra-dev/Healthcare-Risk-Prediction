...
## Decision
We will use **Data Version Control (DVC)** for data and model tracking, defining our workflow via `dvc.yaml`.
...
- **Mitigation:** Setup basic local/remote switching and document `dvc pull` and `dvc repro` in `CONTRIBUTING.md`.
